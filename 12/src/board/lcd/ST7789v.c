...
void LCD_Init(void)
{
    ...
    } else
    {
		...
		if (int_data[0]>=0) 
		{
			...
			sensor_type=int_data[0]>>24&0xff;
    ...
    if(LCD_EN)   //如果有液晶，需要进行初始化
    {
        ...
        //Touch_Adjust();
			  LCD_Clear(WHITE);
				LCD_ShowString(170,8,"setting",BLUE);
				LCD_ShowString(8,24,"running time:0000",BLUE);
				LCD_ShowString(8,40,"mode:press k2 to initialize",BLUE);
				LCD_ShowString(8,64,"DevEui:009569000000F59D",BLUE);
				LCD_ShowString(8,84,"sensor type:",BLUE);
						switch(sensor_type)
								{
			  ...
			  uint16_t x;
				u8 str[]={"00000.00"};//临时数组，用于屏幕显示测量数据
			  x=int_data[0]&0xffff;
				str[0]=(u32)x/10000%10+48;
				str[1]=(u32)x/1000%10+48;
				str[2]=(u32)x/100%10+48;
				str[3]=(u32)x/10%10+48;
				str[4]=(u32)x%10+48;
				str[6]=(u32)(int_data[1])/10+48;
				str[7]=(u32)(int_data[1])%10+48;
				LCD_ShowString(120,104,str,BLUE);//显示测量的平均值
			 // LCD_ShowNum(120,104, x,5,BLUE);
				LCD_ShowString(8,120,"Uploaded data:",BLUE);
				LCD_ShowHexx();
				LCD_ShowString(8,150,"states:",BLUE);
		  	LCD_ShowString(8,170,"back:",BLUE);
		    u8 ch[]={"[UP] Cfm_Stat: 0,UpCnt: 3,CH7: 476700000,DR3: SF9,TxPower: 20,AckCnt: 4,PER: 0,TOA: 205824,nbTrials: 2,BackOff: 0,cRssi: 0,Nb: 0,Band: 7 [DN] Ind_Stat: 0,DnCnt: 4,CH7: 476700000,DR3: SF9,Port: 0,RxSlot: 0,pRSSI: -99,SNR: +1,Pend: 0,SIZE: 0,Multi: 0,Margin: 0,nGW: 0"};
				LCD_Fill(56,170,240,186,WHITE);
				LCD_Fill(0,186,240,320,WHITE);
				LCD_ShowString(56,170,ch,BLUE);
				//LCD_ShowString(16,200,"Uploaded data",BLUE);
				//LCD_ShowString(64,232,"success",BLUE);
				//LCD_ShowString(16,280,"call-back data",BLUE);
    }
}
void LCD_ShowHexx(void)
{
	u8 updata_str[]={"AA F5 9D 01 00 00 00 00 0f"};//临时数组，用于屏幕显示上传的数据
	updata_str[10]=sensor_type+49;//传感器位
	u8 x;
	x=int_data[0]>>16&0xff;
	updata_str[12]=x>>4&0x0f;
	updata_str[13]=x&0x0f;
	x=int_data[0]>>8&0xff;
	updata_str[15]=x>>4&0x0f;
	updata_str[16]=x&0x0f;
	x=int_data[0]&0xff;
	updata_str[18]=x>>4&0x0f;
	updata_str[19]=x&0x0f;
	x=int_data[1];
	updata_str[21]=x>>4&0x0f;
	updata_str[22]=x&0x0f;
	for(u8 i=12;i<23;i++)
	{
		switch(updata_str[i])
		{
